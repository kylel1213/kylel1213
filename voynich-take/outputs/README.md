...
`render_video.py` renders the Beinecke scans read aloud in sync with this WAV (1080p30 MP4): every sounding word lit on its real page, pigments breathing with the pad and bass, the year-clock counting the zodiac labels, the nine rosette rings lit around the foldout, every skipped page riffling past in order. Scans and word boxes are fetched with `python -m visual.fetch --data data --all` on a networked machine; see the project README for the full procedure and the QA sheets.
